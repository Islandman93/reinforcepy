   def _create_input_placeholders(self, input_shape):
        ...
        InputPlaceholders = namedtuple('InputPlaceholders', ['x_input', 'x_input_normalized', 'x_input_tp1',
                                                             'x_input_tp1_normalized', 'x_actions', 'x_rewards', 'x_terminals'])
        return InputPlaceholders(x_input_channel_firstdim, x_input, x_input_tp1_channel_firstdim, x_input_tp1, x_actions,
                                 x_rewards, x_terminals)

    def _create_network_output(self, input_placeholders, output_num, network_generator, create_target_network):
        # Target network does not reuse variables. so we use two different variable scopes
        with tf.variable_scope('network'):
            network_output = network_generator(input_placeholders.x_input_normalized, output_num)
        ...
        if create_target_network:
            with tf.variable_scope('target-network'):
                target_network_output = network_generator(input_placeholders.x_input_tp1_normalized, output_num)
...
# optimizer
        with tf.name_scope('shared-optimizer'):
            ...
            tf_train_step = optimizer.minimize(loss, var_list=network_trainables)
