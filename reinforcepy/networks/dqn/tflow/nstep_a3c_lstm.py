...
class NStepA3CLSTM:
    def __init__(self, input_shape, output_num, optimizer=None, network_generator=None, q_discount=0.99, entropy_regularization=0.01):
        ...
        if network_generator is None:
            network_generator = create_a3c_lstm_network
    ...
    def create_network_graph(self, input_shape, output_num, network_generator, q_discount, optimizer, entropy_regularization):
        ...
        with tf.variable_scope('network'):
            actor_output, critic_output, initial_lstm_state, new_lstm_state = network_generator(x_input, output_num)
            ...
            critic_output = tf.squeeze(critic_output, [1])
        ...
        with tf.name_scope('loss'):
            ...
            with tf.name_scope('actor-entropy'):
                ...
                summarizer.summarize(tf.reduce_sum(actor_entropy), 'scalar', 'actor-entropy')

            with tf.name_scope('actor-loss'):
                # NOTICE: we are summing (accumulating) gradients
                actor_loss = -tf.mul(log_policy_action, tf.stop_gradient(critic_diff))
                summarizer.summarize(tf.reduce_sum(actor_loss), 'scalar', 'actor-loss')
            ...
            with tf.name_scope('critic-loss'):
                ...
                critic_loss = tf.square(critic_diff) / 2
            ...
            with tf.name_scope('total-loss'):
                ...
                total_loss = tf.reduce_sum(critic_loss * 0.5 + actor_loss - (actor_entropy * entropy_regularization))
        ...
        def train_step(sess, current_learning_rate, state, action, reward, lstm_state, summaries=False):
            feed_dict = {x_input_channel_firstdim: state, x_actions: action, x_rewards: reward,
                         tf_learning_rate: current_learning_rate, initial_lstm_state: lstm_state}
            if summaries:
    ...
    def train_step(self, current_learning_rate, state, action, reward, lstm_state, summaries=False):
        return self._train_step(self.tf_session, current_learning_rate, state, action, reward, lstm_state, summaries=summaries)
