...
from reinforcepy.networks.Deepmind import AsyncTargetCNN
from reinforcepy.learners.Deepmind import Async1StepDQNLearner
...
def main(experiment_parameters):
    ...
    cnn = AsyncTargetCNN(network_parameters, training_parameters)
    ...
    cnn_partial = partial(AsyncTargetCNN, network_parameters, training_parameters)

    # create list of learners and the environments to run them on
    learners = list()
    environments = list()
    for learner in range(experiment_parameters.get('learner_count')):
        learner_process = (partial(Async1StepDQNLearner, learner_parameters, cnn_partial), AsyncClientProcess)
