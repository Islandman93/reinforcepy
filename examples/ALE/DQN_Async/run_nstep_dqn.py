...
from reinforcepy.networks.dqn.tflow.nstep_target_dqn import NStepTargetDQN
...
def main(rom_args, learner_args, network_args, num_threads, initial_learning_rate, epochs, logdir, save_interval):
    ...
    network = NStepTargetDQN(input_shape, num_actions, **network_args)

    # create thread host
    thread_host = AsyncTargetThreadHost(network, initial_learning_rate, log_dir=logdir)

    # create threads
    threads = [NStepDQNThreadLearner(environments[t], network, thread_host.shared_dict, **learner_args) for t in range(num_threads)]
